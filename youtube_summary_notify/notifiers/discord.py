"""Discord notifier using Webhooks."""
...
class DiscordNotifier(BaseNotifier):
    ...
    async def send_summary(self, video: VideoInfo) -> bool:
        """Format the summary message and POST to Discord webhook.
    ...
    async def send_error(self, message: str) -> bool:
        """POST an error message to Discord webhook.
    ...
    async def _post(self, text: str) -> bool:
        """Send a text payload to the Discord webhook URL."""
        ...
        try:
            ...
            logger.info("Discord notification sent successfully to target '%s'", self._name)
            return True
        except httpx.HTTPStatusError as exc:
            logger.error(
                "Discord webhook returned %d for target '%s': %s",
            ...
            logger.error("Discord webhook request failed for target '%s': %s", self._name, exc)
            return False
        finally:
